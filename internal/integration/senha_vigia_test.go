...
func TestSenhaVigiaCRUD(t *testing.T) {
	...
	t.Run("List retorna as senhas cadastradas para admin", func(t *testing.T) {
	...
	t.Run("List retorna mensagem padrao para nao-admin", func(t *testing.T) {
		var resp map[string]string
		c.e.reqJSON(http.MethodGet, "/api/v1/usuarios/"+c.vigia.ID.String()+"/senhas", c.vigiaToken, nil, http.StatusOK, &resp)
		if resp["mensagem"] == "" {
			t.Error("vigia deveria receber mensagem padrao, nao a lista de senhas")
		}
	})
	...
	t.Run("Create customizada com nivel ja vinculado retorna 409", func(t *testing.T) {
		nivelID := c.criarNivel(2, 10)
		c.criarSenhaVigia(c.vigia.ID, "customizada", "8100", toUUIDPtr(nivelID.String()))
	...
	t.Run("Update altera codigo de senha", func(t *testing.T) {
		nivelID := c.criarNivel(2, 10)
		senha := c.criarSenhaVigia(c.vigia.ID, "customizada", "8888", toUUIDPtr(nivelID.String()))
	...
	t.Run("Update codigo duplicado retorna 400", func(t *testing.T) {
		...
		senha1 := c.criarSenhaVigia(c.vigia.ID, "customizada", "2100", toUUIDPtr(nivel1.String()))
		c.criarSenhaVigia(c.vigia.ID, "customizada", "2101", toUUIDPtr(nivel2.String()))
	...
	t.Run("Update nao altera codigo para o proprio valor (idempotente)", func(t *testing.T) {
		nivelID := c.criarNivel(2, 10)
		senha := c.criarSenhaVigia(c.vigia.ID, "customizada", "3100", toUUIDPtr(nivelID.String()))
	...
	t.Run("Update altera nivel de senha customizada", func(t *testing.T) {
		nivelOriginal := c.criarNivel(2, 10)
		senha := c.criarSenhaVigia(c.vigia.ID, "customizada", "4100", toUUIDPtr(nivelOriginal.String()))
	...
	t.Run("Update customizada com nivel ja vinculado retorna 409", func(t *testing.T) {
		nivel1 := c.criarNivel(2, 10)
		nivel2 := c.criarNivel(3, 15)
		c.criarSenhaVigia(c.vigia.ID, "customizada", "6100", toUUIDPtr(nivel1.String()))
		senhaB := c.criarSenhaVigia(c.vigia.ID, "customizada", "6101", toUUIDPtr(nivel2.String()))
	...
	t.Run("Delete remove senha customizada", func(t *testing.T) {
		nivelID := c.criarNivel(2, 10)
		senha := c.criarSenhaVigia(c.vigia.ID, "customizada", "7100", toUUIDPtr(nivelID.String()))
