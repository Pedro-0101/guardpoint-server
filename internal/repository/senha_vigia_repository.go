...
func (r *SenhaVigiaRepository) Create(ctx context.Context, s *model.SenhaVigia) error {
	query := `
		INSERT INTO senhas_vigia (empresa_id, usuario_id, tipo, codigo, descricao)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, created_at, updated_at
	`
	if err := r.db.QueryRow(ctx, query,
		s.EmpresaID, s.UsuarioID, s.Tipo, s.Codigo, s.Descricao,
...
func (r *SenhaVigiaRepository) ListByUsuario(ctx context.Context, empresaID, usuarioID uuid.UUID) ([]model.SenhaVigia, error) {
	query := `
		SELECT id, empresa_id, usuario_id, tipo, codigo, descricao, created_at, updated_at
		FROM senhas_vigia
...
func (r *SenhaVigiaRepository) Update(ctx context.Context, id, empresaID uuid.UUID, s *model.SenhaVigia) error {
	query := `
		...
		RETURNING id, empresa_id, usuario_id, tipo, codigo, descricao, created_at, updated_at
	`
	err := r.db.QueryRow(ctx, query, s.Codigo, s.Descricao, id, empresaID).Scan(
		&s.ID, &s.EmpresaID, &s.UsuarioID, &s.Tipo, &s.Codigo, &s.Descricao,
		&s.CreatedAt, &s.UpdatedAt,
	)
	if err != nil {
